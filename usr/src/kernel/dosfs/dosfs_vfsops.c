 ...
 *  October 1992
 *
 * $Id: dosfs_vfsops.c,v 1.1 94/10/19 23:46:51 bill Exp $
 */

static char *dosfs_config = "dosfs 4.";
 ...
 */
	if (error = copyin(p, data, (caddr_t)&args, sizeof(struct pcfs_args)))
		return error;
 ...
 */
	pmp = (struct pcfsmount *)mp->mnt_data;
	copyinstr(p, path, (caddr_t)mp->mnt_stat.f_mntonname,
		sizeof(mp->mnt_stat.f_mntonname)-1, &size);
	memset(mp->mnt_stat.f_mntonname + size, 0,
		sizeof(mp->mnt_stat.f_mntonname) - size);
	/*bzero(mp->mnt_stat.f_mntonname + size,
		sizeof(mp->mnt_stat.f_mntonname) - size);*/
	copyinstr(p, args.fspec, mp->mnt_stat.f_mntfromname, MNAMELEN-1, &size);
	memset(mp->mnt_stat.f_mntfromname + size, 0,
		MNAMELEN - size);
 ...
 */
	vflushbuf(pmp->pm_devvp, waitfor == MNT_WAIT ? B_SYNC : 0, 0);
	return allerror;
}
...
FILESYSTEM_MODCONFIG(dosfs) {
	...
	if (config_scan(dosfs_config, &cfg_string) == 0)
		return;
	if (cfg_number(&cfg_string, &dosfs_vfsops.vfs_type) == 0)
		return;
