static char *isofs_config = "isofs 5.";
...
FILESYSTEM_MODCONFIG(isofs) {
	...
	if (config_scan(isofs_config, &cfg_string) == 0)
		return;
	if (cfg_number(&cfg_string, &isofs_vfsops.vfs_type) == 0)
		return;
...
/*printf(" isofs_mountroot %d ", error);*/
	if (error) {
	...
	imp = VFSTOISOFS(mp);
	(void)memset(imp->im_fsmnt, 0, sizeof(imp->im_fsmnt));
	imp->im_fsmnt[0] = '/';
	imp->iso_ftype = ISO_FTYPE_RRIP;
	(void)memcpy((caddr_t)mp->mnt_stat.f_mntonname, (caddr_t)imp->im_fsmnt,
	    MNAMELEN);
	(void) copystr(ROOTNAME, mp->mnt_stat.f_mntfromname, MNAMELEN - 1,
	    &size);
	(void) memset(mp->mnt_stat.f_mntfromname + size, 0, MNAMELEN - size);
	...
	struct proc *p;
{
	...
	(void) copyinstr(p, path, imp->im_fsmnt, sizeof(imp->im_fsmnt)-1, &size);
	(void) memset(imp->im_fsmnt + size, 0, sizeof(imp->im_fsmnt) - size);
	(void) memcpy((caddr_t)mp->mnt_stat.f_mntonname, (caddr_t)imp->im_fsmnt,
	    MNAMELEN);
...
/*printf(" open ");*/
#endif
	if (error = VOP_OPEN(devvp, (ronly ? FREAD : FREAD|FWRITE) | FMOUNT, NOCRED, p))
		return (error);
	needclose = 1;
...
/*printf(" type %x ", vdp->type);*/
	...
	(void) memcpy(isomp->root, rootp, sizeof isomp->root);
	...
	struct vnode **vpp;
{
	...
	error = iso_iget(ip, iso_lblktosize(imp, imp->root_extent), &nip,
	...
	struct proc *p;
{
	...
	if (sbp != &mp->mnt_stat) {
		(void) memcpy((caddr_t)&sbp->f_mntonname[0],
			(caddr_t)mp->mnt_stat.f_mntonname, MNAMELEN);
		(void) memcpy((caddr_t)&sbp->f_mntfromname[0],
	...
	struct fid *fhp;
{
	...
	if(ip->i_number == iso_lblktosize(mp, mp->root_extent)) {
