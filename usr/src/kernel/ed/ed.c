 ...
 */
	...
	int	alive;		/* unit exists and was attached */
 ...
 *	(*bogus*, since many clone cards decode inconsistant number, thus
 *       trying to detect conflicts is a "pipe dream" for [E]ISA -wfj)
 */
int
ed_probe(isa_dev)
	struct isa_device *isa_dev;
{
	...
	/* wildcard unit? */
	if (isa_dev->id_unit == '?')
		isa_dev->id_unit = lastunit;
	if (isa_dev->id_unit >= ned)
		return (0);
	sc = &ed_softc[isa_dev->id_unit];
	
	while (sc->alive) {
		if (sc->alive && isa_dev->id_unit > ned)
			return(0);
		isa_dev->id_unit++;
		sc++;
	}
	...
	struct isa_device *isa_dev;
{
	...
	if ((ifa != 0) && (ifa->ifa_addr != 0)) {
		...
		sdl->sdl_slen = 0;
		memcpy(LLADDR(sdl), sc->arpcom.ac_enaddr, ETHER_ADDR_LEN);
	}
	...
	int unit;
{
	...
	/*
	 * Sledge hammer for NE-2000; reset entire board, otherwise, it
	 * will ignore reinitialization and jam bus on next packet send.
	 */
	if (sc->mem_shared == 0) {
		u_char tmp;

		/* reset the board ala probe() below */
		tmp = inb(sc->asic_addr + ED_NOVELL_RESET);
	...
	case SIOCSIFADDR:
		...
		case AF_NS:
			...
			if (ns_nullhost(*ina))
				 ...
				 * 
				 */
				memcpy((caddr_t)sc->arpcom.ac_enaddr,
...
/*#if NBPFILTER > 0*/
	...
	if (sc->bpf) {
		...
		if ((sc->arpcom.ac_if.if_flags & IFF_PROMISC) &&
			memcmp(eh->ether_dhost, sc->arpcom.ac_enaddr,
...
DRIVER_MODCONFIG(ed) {
	...
	if (!config_scan(ed_config, &cfg_string)) 
		return;

	/* allocate controller resources */
	ned = 1;
	(void)cfg_number(&cfg_string, &ned);
	ed_softc = (struct ed_softc *)
	    malloc(sizeof(struct ed_softc) * ned, M_TEMP, M_NOWAIT);
	(void)memset((void *)ed_softc, 0, sizeof (struct ed_softc) * ned);

	/* XXX: configure driver into kernel program  (done by probe/attach) */

	/* probe for hardware */
	new_isa_configure(&cfg_string, &eddriver);
}
